...
import 'package:xinxiangqin/home/home_shaixuan_page.dart';
...
import 'package:xinxiangqin/home/qingshaonian_dialog.dart';
import 'package:xinxiangqin/home/qingshaonian_open.dart';
import 'package:xinxiangqin/home/tags_future_nochoose.dart';
...
//星座图片列表
  ...
  getUpdateInfo()async{
    ...
    // service.get(Apis.getNewVersionRecord, successCallback: (data) async {
    //   if(mounted){
    //     setState(() {
    //       log('app更新信息'+data.toString());
    //       jumpDownloadWeb(data['downloadLink'].toString());
    //     });
    //   }
    // }, failedCallback: (data) {
    //   log('app更新信息报错'+data.toString());
    // });

  }

  jumpDownloadWeb(url)async{
    showDialog(
        context: context,
  ...
  showQingshaonian()async{
    SharedPreferences share = await SharedPreferences.getInstance();
    String? qingshaonianmoshiFirst = share.getString('qingshaonianmoshiFirst');
    String? qingshaonianmoshi = share.getString('qingshaonianmoshi');
    if (qingshaonianmoshi=='true'){
      ...
      if(qingshaonianmoshiFirst!='false'){
        await share.setString('qingshaonianmoshiFirst', 'false');
        setState(() {
  ...
  getuserTuijian() async{
    ...
    if (agestr==''||agestr == 'null'||agestr==null){
      requestData = {
        'auditStatus': '1',
        'appType': '1'
      };
      ...
      requestData = {
        ...
        'appType': '1',
        'ageRange':agestr,
        'heightRange':heightstr,
        'weightRange':weightstr,
      };
    }
    requestData = {
      'auditStatus': '1',
    ...
    log('传值的参数'+requestData.toString());


    NetWorkService service = await NetWorkService().init();
    service.get(Apis.getMatchBlindMembers, queryParameters: requestData,
        successCallback: (data) async {
      log('推荐数据'+data.toString());
       // recordTime();
  ...
  //浏览过的次数
  liulanNum()async{
    print('浏览的id'+ListData[selectRow]['id'].toString());
    NetWorkService service = await NetWorkService().init();
    service.post(Apis.businessCardBrowsing, data: {
      'usedBy': ListData[selectRow]['id'],
    },
        successCallback: (data) async {
  ...
  netChile()async{
    ...
    if (selectRow<ListData.length-1){
  ...
  chooseLike() async{
    ...
    service.post(Apis.likeMatchBlindMembers, data: map, successCallback: (data) async {
      ...
      // sendKefuMessage(userdic11);
      log('喜欢接口传值'+map.toString());
      log('喜欢接口返回'+data.toString());
  ...
  sendKefuMessage1(data)async{
    ...
    if (createTextMessageRes.code == 0) {
      ...
      if (sendMessageRes.code == 0) {
        // 发送成功
        // rightToChat1(data);
        BotToast.showText(text: '已设置超级喜欢，等待对方回复即可开启聊天');
      }else{
        getMyvipQuanyi();
      }
    }
  ...
  initTagList()async{
    ...
    setState(() {
      ...
      if (ListData[selectRow]['expectedLabelDOS']==null){
        tagListFuture = [];
      }else{
        tagListFuture = ListData[selectRow]['expectedLabelDOS'];
      }

      userInfoDic = ListData[selectRow];
      if (ListData[selectRow]['getDistance'] == null||ListData[selectRow]['getDistance'].toString() == 'null'){
        juli = '';
      }else{
        juli = ListData[selectRow]['getDistance'];
      }
      ...
      //抽烟
      String str9 = ListData[selectRow]['smokerName'].toString();
      Map map8 = {};
      map8['label'] = str9;
      newTagList.add(map8);

      //喝酒
      String str10 = ListData[selectRow]['drinkName'].toString();
      Map map9 = {};
      map9['label'] = str10;
      newTagList.add(map9);

      log('某一条推荐数据'+ListData[selectRow].toString());
      tagList = newTagList;
      createBlindMemberSeeMe(ListData[selectRow]);
      liulanNum();
    });
  ...
  Future<void> rightMain() async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();

    await sharedPreferences.setString('age','');
    await sharedPreferences.setString('height','');
    await sharedPreferences.setString('weight','');
    // String? heightstr = sharedPreferences.getString('height');
    // String? weightstr = sharedPreferences.getString('weight');
    BotToast.showLoading();
    ...
    String? locationShouquan = sharedPreferences.getString('locationShouquan');
    if (locationShouquan=='true'){
      setState(() {
  ...
  Future<void> requestLocationPermission() async {
    ...
    showDialog(
      ...
      builder: (BuildContext context){
        return AlertDialog(
          title: Text('权限申请'),
          content: Text('西瓜岛将通过位置信息为您提供距离测算服务'),
          actions: <Widget>[
            TextButton(
        ...
        child: Text('确定'),
              onPressed: ()async {
                Navigator.of(context).pop();
                // 申请结果处理
                sharedPreferences.setString('locationShouquan', 'true');
                ...
                if (data) {
                print('初始化成功');
                await sharedPreferences.setString('locationShouquan', 'true');
                setState(() {
                positionShouquan = true;
                getLocation();
                });
    ...
    //     await sharedPreferences.setString('locationShouquan', 'false');
    //     setState(() {
    //       positionShouquan = false;
    //     });
    //     // getMyvipQuanyi();
    //
    //   }
  ...
  createBlindMemberSeeMe(data)async{
    ...
    // BotToast.showLoading();
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    log('我看过谁传值'+'issue'+sharedPreferences.get('UserId').toString()+'receive'+data['id'].toString());
     ...
     Container(
       ...
       child:Stack(
         children: [
           ...
           qingshaonianMoshi!=true?Positioned(
             top: MediaQuery.of(context).padding.top,
             right: 15,
             child:GestureDetector(
               onTap: ()async{
                 await Navigator.push(context,
                     MaterialPageRoute(builder: (BuildContext ccontext) {
                       return HomeShaixuanPage();
                     }));
                 firstgetMyvipQuanyi();

               },
               child: Image(image: AssetImage('images/home/shaixuan.png'),width: 20.5,height: 20,),
             ),
           ):Container(),
                                   ...
                                   children: [
                                     ...
                                     Stack(
                                       children: [
                                         ...
                                         userInfoDic['avatar']!=null&&userInfoDic['avatar'].toString()!=''?Container(
                                               ...
                                               child: Row(
                                                 ...
                                                 children: [
                                                   ...
                                                   Text(
                                                     ...
                                                     ListData.length!=0&&juli!=''&&juli!='距离未知'&&juli!='null'&&positionShouquan==true?'距你'+juli+'km':'距离未知',
...
//                                  ),
                                 ...
                                 Container(
                                   margin: const EdgeInsets.only(top: 30,right: 9.5,left: 9.5),
                                   child: Row(
               ...
               child: Container(
                 ...
                 child: Column(
                   ...
                   children: [
                     ...
                     qingshaonianMoshi!=true?Container(
                       margin: EdgeInsets.only(bottom: 17.5,right: 15),
                       child: GestureDetector(
                         onTap: (){
                           //超级喜欢
                   ...
                   child: SingleChildScrollView(
                     child:Container(
                         child:  Column(
                           crossAxisAlignment: CrossAxisAlignment.start,
                           children: [
                             Text('西瓜岛',style: TextStyle(color: Color(0xff333333),fontSize: 19.5,fontWeight: FontWeight.bold),),
                             SizedBox(height: 15,),
                             Row(
                               mainAxisAlignment: MainAxisAlignment.start,
                               children: [
                                 ...
                                 Text('，明天会有更多新朋友',style: TextStyle(color: Color(0xff333333),fontSize: 16,fontWeight: FontWeight.w500),)
                               ],
                             ),
                             ...
                             Container(
                               ...
                               child: Column(
                                 ...
                                 children: [
                                   ...
                                   Container(
                                     margin: EdgeInsets.only(left: 31),
                                     child: Text('明天的新朋友会更好~',style: TextStyle(color: Colors.white,fontSize: 18,fontWeight: FontWeight.bold),),
                                   ),
                                   // SizedBox(height: 5,),
                                   Container(
                                     margin: EdgeInsets.only(left: 33),
                                     child: Text('我更懂你啦，会有更多新朋友喜欢你',style: TextStyle(color: Color(0xffFEFEFE),fontSize: 11,fontWeight: FontWeight.normal),),
                                   ),
                             ...
                             GestureDetector(
                               onTap: ()async{
                                 userDic['haveNameAuth']!=1?rightNameAuth():BotToast.showText(text: '您已完成实名');
                               },
                               child: Container(
                                   alignment: Alignment.center,
                                   margin: EdgeInsets.only(left:0,right: 15,top: 15),
